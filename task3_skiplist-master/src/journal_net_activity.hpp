...
{
	NodeSkipList<NetActivity, TimeStamp, numLevels>* startNode = _journal.findFirst(timeFrom);

	while (startNode->key <= timeTo && startNode != _journal.getPreHead())
	{
		if (startNode->value.host == hostSuspicious)
			std::cout << startNode->key << " " << startNode->value << std::endl;
